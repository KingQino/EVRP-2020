...
def main():
    ...
    print(f'The optimal result cost from my MA: {best_cost}')
    print(best_solution)

    visualize(training_file)
    visualize_routes(best_solution, df, 'MA with complex local search operators Best Solution for ' + instance.name, is_show=False, is_save=True, save_path=RESULT_DIR + instance.name.split('.')[0] + '-routes.png')
...
def visualize(training_file):
    ...
    title = instance_name + ' | MA with complex local search operators'
